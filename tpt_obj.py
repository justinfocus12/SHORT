...
class TPT:
    ...
    def plot_field_long_2d(self,model,data,fieldnames,field_funs,field_abbs,units=[1.0,1.0],tmax=70,field_unit_symbols=["",""],orientation=None):
        ...
        del x_long
        return 
    def plot_mfpt_fidelity(self,q,dirn=1,laptrans_flag=False):
        ss = np.random.choice(np.arange(self.nshort),size=min(self.nshort,100000),replace=False)
        if dirn==1:
            nn = np.where((np.isnan(self.short_tau_a_fwd)==0)*(np.isnan(self.short_tau_b_fwd)==0))[0]
            print("nn.shape = {}".format(nn.shape))
            ss = np.intersect1d(ss,nn)
            emp_mfpt_a = self.short_tau_a_fwd[ss]
            emp_mfpt_b = self.short_tau_b_fwd[ss]
            if laptrans_flag:
                mfpt_a = self.mfpt_a_moment1_fwd[ss,0]
                mfpt_b = self.mfpt_b_moment1_fwd[ss,0]
            else:
                mfpt_a = self.mfpt_a_x[ss,0]
                mfpt_b = self.mfpt_b_x[ss,0]
        else:
            nn = np.where((np.isnan(self.short_tau_a_bwd)==0)*(np.isnan(self.short_tau_b_bwd)==0))[0]
            print("nn.shape = {}".format(nn.shape))
            ss = np.intersect1d(ss,nn)
            emp_mfpt_a = self.short_tau_a_bwd[ss]
            emp_mfpt_b = self.short_tau_b_bwd[ss]
            if laptrans_flag:
                mfpt_a = self.mfpt_a_moment1_bwd[ss,0]
                mfpt_b = self.mfpt_b_moment1_bwd[ss,0]
            else:
                mfpt_a = self.mfpt_a_bwd[ss,0]
                mfpt_b = self.mfpt_b_bwd[ss,0]
        print("in plot_mfpt_fidelity. emp_mfpt_a range = ({:3.3e},{:3.3e}), mfpt_a range = ({:3.3e},{:3.3e})".format(np.nanmin(emp_mfpt_a),np.nanmax(emp_mfpt_a),np.min(mfpt_a),np.max(mfpt_a)))
        print("in plot_mfpt_fidelity. emp_mfpt_b range = ({:3.3e},{:3.3e}), mfpt_b range = ({:3.3e},{:3.3e})".format(np.nanmin(emp_mfpt_b),np.nanmax(emp_mfpt_b),np.min(mfpt_b),np.max(mfpt_b)))
        weights = np.ones(len(ss))/len(ss)
        symbol = '+' if dirn==1 else '-'
        # ------
        # diagnose
        print("mfpt_a summary stats: mean={}, std={}, min={}, max={}".format(np.mean(mfpt_a),np.std(mfpt_a),np.min(mfpt_a),np.max(mfpt_a)))
        print("emp_mfpt_a summary stats: mean={}, std={}, min={}, max={}".format(np.nanmean(emp_mfpt_a),np.nanstd(emp_mfpt_a),np.nanmin(emp_mfpt_a),np.nanmax(emp_mfpt_a)))
        fig,ax = plt.subplots(ncols=2,figsize=(12,6))
        ax[0].hist(mfpt_a)
        ax[0].set_title(r"DGA $E[\tau_A^%s]$ histogram"%symbol)
        ax[1].hist(emp_mfpt_a)
        ax[1].set_title(r"Empirical $E[\tau_A^%s]$ histogram"%symbol)
        fig.savefig(join(self.savefolder,"mfpt_a_{}_histogram".format(dirn)))
        plt.close(fig)
        if laptrans_flag:
            title = r"$E_x[\tau_A^%s]$ (L.T.) fidelity (days)"%symbol
            funname = r"DGA $\partial_\lambda E_x[\exp(\lambda\tau_A^%s)]$"%symbol 
        else:
            title = r"$\tau_A^%s$ fidelity"%symbol
            funname = r"DGA E_x[\tau_A^%s]$"%symbol
        fig,ax,_ = self.plot_field_1d(emp_mfpt_a,weights,mfpt_a,shp=[20,],funname=funname,fieldname=r"Empirical $E_x[\tau_A^%s]$ (days)"%symbol,color='black',linestyle='None')
        if dirn==-1 and laptrans_flag==False:
            print("xlim = {}".format(ax.get_xlim()))
            print("ylim = {}".format(ax.get_ylim()))
        mfpt_max = max(ax.get_xlim()[1],ax.get_ylim()[1])
        mfptrange = np.array([0,mfpt_max])
        ax.set_xlim(mfptrange)
        ax.set_ylim(mfptrange)
        #mfptrange = np.array([np.min(self.mfpt_a_x),np.max(self.mfpt_a_x)])
        ax.plot(mfptrange,mfptrange,color='black',linestyle='--')
        ax.set_title(title,fontdict=font)
        fig.savefig(join(self.savefolder,"mfpt_a_%d_lt%d_fidelity"%(dirn,1*laptrans_flag)))
        plt.close(fig)
        if laptrans_flag:
            title = r"$E_x[\tau_B^%s]$ (L.T.) fidelity (days)"%symbol
            funname = r"DGA $\partial_\lambda E_x[\exp(\lambda\tau_B^%s)]$"%symbol 
        else:
            title = r"$\tau_B^%s$ fidelity"%symbol
            funname = r"DGA E_x[\tau_B^%s]$"%symbol
        fig,ax,_ = self.plot_field_1d(emp_mfpt_b,weights,mfpt_b,shp=[20,],funname=funname,fieldname=r"Empirical $\tau_B^%s$ (days)"%symbol,color='black',linestyle='None')
        mfpt_max = max(ax.get_xlim()[1],ax.get_ylim()[1])
        mfptrange = np.array([0,mfpt_max])
        ax.set_xlim(mfptrange)
        ax.set_ylim(mfptrange)
        ax.plot(mfptrange,mfptrange,color='black',linestyle='--')
        ax.set_title(title,fontdict=font)
        fig.savefig(join(self.savefolder,"mfpt_b_%d_lt%d_fidelity"%(dirn,1*laptrans_flag)))
        plt.close(fig)
        print("Done plotting MFPT fidelity")
        return
    def plot_chom_fidelity_multiple(self,data,q):
        # Empirically validate the change of measure for multiple observables
        key0_list,key1_list,theta_fun0_list,theta_fun1_list,theta0name_list,theta1name_list,fname_suffix_list = self.canonical_2d_observables(q)
        for i in range(len(key0_list)):
            fig,ax = self.plot_chom_fidelity(data,q,[theta_fun0_list[i],theta_fun1_list[i]])
            ax[0].set_xlabel(r"$\frac{d\pi}{d\mu}$ DGA")
            ax[0].set_ylabel(r"$\frac{d\pi}{d\mu}$ Empirical")
            ax[0].set_title(r"Subspace (%s, %s)"%(theta0name_list[i],theta1name_list[i]))
            fig.savefig(join(self.savefolder,"chom_fidelity_%s"%(fname_suffix_list[i])))
            plt.close(fig)
        return
    def plot_chom_fidelity(self,data,q,theta_list=None):
        # Estimate empirical change of measure and compare with the calculated one
        ss = np.random.choice(np.arange(self.nshort),size=500000,replace=True)
        if theta_list == None:
            funlib = hm.observable_function_library(q)
            theta_list = [fun_at_level(funlib["mag"]["fun"],self.ref_alt,q)]
            theta_list += [fun_at_level(funlib["U"]["fun"],self.ref_alt,q)]
        theta_x = np.zeros((len(ss),len(theta_list)))
        for i in range(len(theta_list)):
            theta_x[:,i] = theta_list[i](data.X[ss,0]).flatten()
        print("theta_x.shape = {}".format(theta_x.shape))
        print("ss.shape = {}".format(ss.shape))
        shp,dth,thaxes,cgrid,chom_dga,field_std,field_std_L2,field_std_Linf,bounds = self.project_field(self.chom[ss]/np.sum(self.chom[ss]),np.ones(len(ss)),theta_x,avg_flag=False)
        print("chom_dga: mean={}, min={}, max={}".format(np.nanmean(chom_dga),np.nanmin(chom_dga),np.nanmax(chom_dga)))
        x_long = load(join(self.long_simfolder,"x_long.npy"))
        ss_emp = np.random.choice(np.arange(len(x_long)),size=1000000,replace=True)
        x_long_ss = x_long[ss_emp]
        del x_long
        theta_x_emp = np.zeros((len(ss_emp),len(theta_list)))
        for i in range(len(theta_list)):
            theta_x_emp[:,i] = theta_list[i](x_long_ss).flatten()
        _,_,_,_,chom_emp,_,_,_,_ = self.project_field(np.ones(len(ss_emp))/len(ss_emp),np.ones(len(ss_emp)),theta_x_emp,avg_flag=False,shp=shp,bounds=bounds)
        print("chom_emp: mean={}, min={}, max={}".format(np.nanmean(chom_emp),np.nanmin(chom_emp),np.nanmax(chom_emp)))
        fig,ax = plt.subplots(ncols=3,figsize=(18,6))
        ax[0].scatter(chom_dga,chom_emp,marker='o',color='black')
        chmin = min(np.nanmin(chom_dga),np.nanmin(chom_emp))
        chmax = max(np.nanmax(chom_dga),np.nanmax(chom_emp))
        chmin = 1e-8
        chmax = 1.0
        ax[0].set_xlim([chmin,chmax])
        ax[0].set_ylim([chmin,chmax])
        ax[0].plot([chmin,chmax],[chmin,chmax],linestyle='--',color='black')
        ax[0].set_xscale('log')
        ax[0].set_yscale('log')
        _,_ = self.plot_field_2d(data,self.chom[ss],np.ones(self.nshort)[ss],theta_x,avg_flag=False,cmap=plt.cm.jet,fieldname=r"DGA $\pi$",std_flag=False,current_flag=False,dirn=0,logscale=True,ss=ss,fig=fig,ax=ax[1])
        _,_ = self.plot_field_2d(data,np.ones(len(ss_emp))/len(ss_emp),np.ones(len(ss_emp)),theta_x_emp,avg_flag=False,cmap=plt.cm.jet,fieldname=r"Empirical $\pi$",std_flag=False,current_flag=False,dirn=0,logscale=True,fig=fig,ax=ax[2])
        return fig,ax
    ...
    def tabulate_functions(self,data,theta_list,current_flag=True,ss=None):
        ...
        return ans
    def compare_conditional_lead_times(self,q,data):
        funlib = hm.observable_function_library(q)
        theta_x_0 = fun_at_level(funlib["U"]["fun"],self.ref_alt,q)(data.X[:,0])
        theta_x_1 = self.comm_fwd[:,0:1]
        print("theta_x_0.shape={}, theta_x_1.shape={}, self.chom.shape={}".format(theta_x_0.shape,theta_x_1.shape,self.chom.shape))
        funname0 = funlib["U"]["name"] + " ({} km)".format(self.ref_alt)
        funname1 = r"$q^+$"
        units0 = funlib["U"]["units"]
        units1 = 1.0
        unit_symbol0 = funlib["U"]["unit_symbol"]
        unit_symbol1 = ""
        fig,ax = self.compare_lead_times(self.chom,theta_x_0.flatten(),fieldname=funname0,units=units0,unit_symbol=unit_symbol0)
        fig.savefig(join(self.savefolder,"mfpt_b_ava_clt_U_q"))
        plt.close(fig)
        return
    def canonical_2d_observables(self,q):
        funlib = hm.observable_function_library(q)
        key0_list = []
        key1_list = []
        theta_fun0_list = []
        ...
        key0 = "mag"
        key1 = "U"
        altitude0 = self.ref_alt
        altitude1 = self.ref_alt
        fname_suffix = ("%s%f_%s%f" % (key0,altitude0,key1,altitude1)).replace(".","p")
        fname_suffix_list += [fname_suffix]
        key0_list += [key0]
        key1_list += [key1]
        theta_fun0_list += [fun_at_level(funlib[key0]["fun"],altitude0,q)]
        theta_fun1_list += [fun_at_level(funlib[key1]["fun"],altitude1,q)]
        theta0name_list += [funlib[key0]["name"]+" ({} km)".format(altitude0)]
        theta1name_list += [funlib[key1]["name"]+" ({} km)".format(altitude1)]
        key0 = "mag"
        key1 = "U"
        altitude0 = 21.5
        altitude1 = 21.5
        ...
        key0 = "q2"
        key1 = "dqdy"
        fname_suffix = ("%smean_%smean" % (key0,key1)).replace(".","p")
        fname_suffix_list += [fname_suffix]
        key0_list += [key0]
        key1_list += [key1]
        theta_fun0_list += [fun_zmean(funlib[key0]["fun"],q)] 
        theta_fun1_list += [fun_zmean(funlib[key1]["fun"],q)] 
        theta0name_list += [funlib[key0]["name"]+" z-mean"]
        theta1name_list += [funlib[key1]["name"]+" z-mean"]
        return key0_list,key1_list,theta_fun0_list,theta_fun1_list,theta0name_list,theta1name_list,fname_suffix_list
    def plot_projections_1d_array(self,model,data):
        ...
        plt.close(fig)
        return
    def plot_projections_1d(self,model,data):
        q = model.q
        funlib = model.observable_function_library()
        #theta1d_list = ['U','impsi','vT','LASSO','vTint']
        #altitude_list = [21.5,model.ref_alt]
        theta1d_list = ['Uref','vTintref','LASSO']
        n = q['Nz']-1
        # Plot the 1D projections
        ss = np.random.choice(np.arange(self.nshort),size=min(self.nshort,100000),replace=False)
        ...
        mfpt_b = self.mfpt_b #self.dam_moments['one']['xb'][1]
        mfpt_a = self.mfpt_a #self.dam_moments['one']['xa'][1]
        eps = 1e-5
        mfpt_xb = self.dam_moments['one']['xb'][1,:,0]*(comm_fwd[:,0] > eps)/(comm_fwd[:,0] + 1*(comm_fwd[:,0] <= eps))
        mfpt_xb[np.where(comm_fwd[:,0] <= eps)[0]] = np.nan
        mfpt_xa = self.dam_moments['one']['xa'][1,:,0]*(1-comm_fwd[:,0] > eps)/(1-comm_fwd[:,0] + 1*(1-comm_fwd[:,0] <= eps))
        mfpt_xa[np.where(1-comm_fwd[:,0] <= eps)[0]] = np.nan
        for i in range(len(theta1d_list)):
            if theta1d_list[i] == 'LASSO':
                funname = 'LASSO'
                theta_x = theta_x = (self.lasso_predict_allz(model,data.X[ss,0,:])).reshape((len(ss),1))
            ...
            # Committor
            handles = []
            fig,ax,handle = self.plot_field_1d(comm_fwd[ss,0],chomss,theta_x,fieldname=r"$q^+$ (probability)",funname=funname,std_flag=True,label=r"$q^+$",color='black',units=units,unit_symbol=unit_symbol) 
            ...
            #if theta1d_list[i] != 'Uref':
            #    ax.yaxis.set_major_locator(ticker.NullLocator())
            #    ax.yaxis.set_visible(False)
            fig.savefig(join(self.savefolder,"comm_fwd{}".format(file_suffix)))
            plt.close(fig)
            fig,ax,handle = self.plot_field_1d(comm_bwd[ss,0],chomss,theta_x,fieldname=r"$q^-$ (probability)",funname=funname,std_flag=True,label=r"$q^-$",color='blue',units=units,unit_symbol=unit_symbol)
            #ax.legend(handles=[handle])
            ax.set_title(r"$q^-$",fontdict=font)
            fig.savefig(join(self.savefolder,"comm_bwd{}".format(file_suffix)))
            plt.close(fig)
            # MFPT 
            # tau_B^+
            fig,ax,handle = self.plot_field_1d(mfpt_b[ss,0],chomss,theta_x,fieldname=r"$E[\tau_B]$ (days)",funname=funname,std_flag=True,label=r"$E[\tau_B]$",color='black',units=units,unit_symbol=unit_symbol) 
            if theta1d_list[i] == 'vT': 
                ax.xaxis.set_major_formatter(ticker.FuncFormatter(sci_fmt))
            ax.xaxis.set_major_locator(ticker.MaxNLocator(nbins=1))
            ax.yaxis.set_major_locator(ticker.MaxNLocator(nbins=3))
            ...
            #    ax.yaxis.set_major_locator(ticker.NullLocator())
            #    ax.yaxis.set_visible(False)
            fig.savefig(join(self.savefolder,"mfpt_b_fwd{}".format(file_suffix)))
            plt.close(fig)
            # tau_A^+
            fig,ax,handle = self.plot_field_1d(mfpt_a[ss,0],chomss,theta_x,fieldname=r"$E[\tau_A]$ (days)",funname=funname,std_flag=True,label=r"$E[\tau_A]$",color='black',units=units,unit_symbol=unit_symbol) 
            if theta1d_list[i] == 'vT': 
                ax.xaxis.set_major_formatter(ticker.FuncFormatter(sci_fmt))
            ax.xaxis.set_major_locator(ticker.MaxNLocator(nbins=1))
            ax.yaxis.set_major_locator(ticker.MaxNLocator(nbins=3))
            if theta1d_list[i] != 'U':
                ax.yaxis.set_major_locator(ticker.NullLocator())
                ax.yaxis.set_visible(False)
            fig.savefig(join(self.savefolder,"mfpt_a_fwd{}".format(file_suffix)))
            plt.close(fig)
            # Conditional MFPT
            # tau_B^+
            fig,ax,handle = self.plot_field_1d(mfpt_xb[ss],chomss,theta_x,fieldname=r"$E[\tau_B|x\to B]$ (days)",funname=funname,std_flag=True,label=r"$E[\tau_B|x\to B]$",color='black',units=units,unit_symbol=unit_symbol) 
            if theta1d_list[i] == 'vT': 
                ax.xaxis.set_major_formatter(ticker.FuncFormatter(sci_fmt))
            ax.xaxis.set_major_locator(ticker.MaxNLocator(nbins=1))
            ax.yaxis.set_major_locator(ticker.MaxNLocator(nbins=3))
            ...
            #    ax.yaxis.set_major_locator(ticker.NullLocator())
            #    ax.yaxis.set_visible(False)
            fig.savefig(join(self.savefolder,"mfpt_xb_fwd{}".format(file_suffix)))
            plt.close(fig)
            # tau_A^+
            fig,ax,handle = self.plot_field_1d(mfpt_xa[ss],chomss,theta_x,fieldname=r"$E[\tau_A|x\to A]$ (days)",funname=funname,std_flag=True,label=r"$E[\tau_A|x\to A]$",color='black',units=units,unit_symbol=unit_symbol) 
            if theta1d_list[i] == 'vT': 
                ax.xaxis.set_major_formatter(ticker.FuncFormatter(sci_fmt))
            ax.xaxis.set_major_locator(ticker.MaxNLocator(nbins=1))
            ax.yaxis.set_major_locator(ticker.MaxNLocator(nbins=3))
            if theta1d_list[i] != 'U':
                ax.yaxis.set_major_locator(ticker.NullLocator())
                ax.yaxis.set_visible(False)
            fig.savefig(join(self.savefolder,"mfpt_xa_fwd{}".format(file_suffix)))
            plt.close(fig)
            # Densities
            handles = []
            orientation = 'vertical' if theta1d_list[i]=='U' else 'horizontal'
            fig,ax,handle = self.plot_field_1d(chomss,np.ones(self.nshort)[ss],theta_x,avg_flag=False,fieldname=r"Prob. density",funname=funname,label=r"$\pi$ (prob. density)",color='black',units=units,unit_symbol=unit_symbol,orientation=orientation) 
            if theta1d_list[i] == 'U':
                ax.set_ylim([np.min(theta_x)*units,np.max(theta_x)*units])
            handles += [handle]
            #piab = self.comm_fwd[ss,0]*self.comm_bwd[ss,0]*chomss
            #piab *= 1.0/np.sum(piab)
            #_,_,handle = self.plot_field_1d(piab,np.ones(self.nshort)[ss],theta_x,avg_flag=False,fieldname="",funname=funname,label=r"$\pi_{AB}$",color='red',fig=fig,ax=ax,units=units,unit_symbol=unit_symbol,orientation=orientation) 
            #handles += [handle]
            #piba = (1-self.comm_fwd[ss,0])*(1-self.comm_bwd[ss,0])*chomss
            #piba *= 1.0/np.sum(piba)
            #_,_,handle = self.plot_field_1d(piba,np.ones(self.nshort)[ss],theta_x,avg_flag=False,fieldname="",funname=funname,label=r"$\pi_{BA}$",color='skyblue',fig=fig,ax=ax,units=units,unit_symbol=unit_symbol,orientation=orientation) 
            #handles += [handle]
            ax.legend(handles=handles)
            ax.set_title("Density on {}".format(funname),fontdict=font)
            fig.savefig(join(self.savefolder,"dens{}".format(file_suffix)))
            plt.close(fig)
        if True: return
        #sys.exit()
        ...
        theta0name_list += [funlib[key0]["name"]+" z-mean"]
        theta1name_list += [funlib[key1]["name"]+" z-mean"]
        # Get least action path
        xfw_ab = np.load(join(self.fwfolder,"xmin_AB.npy"))
        tfw_ab = np.load(join(self.fwfolder,"tmin_AB.npy"))
        tsubset_fw_ab = np.linspace(0,len(tfw_ab)-1,300).astype(int)
        xfw_ab = xfw_ab[tsubset_fw_ab]
        tfw_ab = tfw_ab[tsubset_fw_ab]
        xfw_ba = np.load(join(self.fwfolder,"xmin_BA.npy"))
        tfw_ba = np.load(join(self.fwfolder,"tmin_BA.npy"))
        tsubset_fw_ba = np.linspace(0,len(tfw_ba)-1,300).astype(int)
        xfw_ba = xfw_ba[tsubset_fw_ba]
        tfw_ba = tfw_ba[tsubset_fw_ba]
        tsubset_fw_ba = np.linspace(0,len(tfw_ba)-1,300).astype(int)
        for i in range(len(key0_list)):
            key0 = key0_list[i]
            key1 = key1_list[i]
            units = np.array([funlib[key0]["units"],funlib[key1]["units"]])
            unit_symbols = [funlib[key0]["unit_symbol"],funlib[key1]["unit_symbol"]]
            fun0name = theta0name_list[i]
            fun1name = theta1name_list[i]
            magu_ab = np.zeros((2,2))
            magu_ab[:,0] = theta_fun0_list[i](np.array([self.acv_compute,self.bcv_compute])).flatten()
            magu_ab[:,1] = theta_fun1_list[i](np.array([self.acv_compute,self.bcv_compute])).flatten()
            ...
            magu_obs_ab = []
            magu_obs_ba = []
            for ti in range(num_trans):
                idx0,idx1 = ab_starts[ti],ab_ends[ti]
                moab = np.zeros((idx1-idx0,2))
                moab[:,0] = theta_fun0_list[i](x_long[idx0:idx1]).flatten()
                moab[:,1] = theta_fun1_list[i](x_long[idx0:idx1]).flatten()
                magu_obs_ab += [moab]
                idx0,idx1 = ba_starts[ti],ba_ends[ti]
                moba = np.zeros((idx1-idx0,2))
                moba[:,0] = theta_fun0_list[i](x_long[idx0:idx1]).flatten()
                moba[:,1] = theta_fun1_list[i](x_long[idx0:idx1]).flatten()
                magu_obs_ba += [moba]
            del x_long
            # Least action paths
            magu_fw_ab = np.zeros((len(xfw_ab),2))
            magu_fw_ab[:,0] = theta_fun0_list[i](xfw_ab).flatten()
            magu_fw_ab[:,1] = theta_fun1_list[i](xfw_ab).flatten()
            magu_fw_ba = np.zeros((len(xfw_ba),2))
            magu_fw_ba[:,0] = theta_fun0_list[i](xfw_ba).flatten()
            magu_fw_ba[:,1] = theta_fun1_list[i](xfw_ba).flatten()
            # currents
            magu_x,magu_yj,magu_xpj,magu_ypj = self.tabulate_functions(data,[theta_fun0_list[i],theta_fun1_list[i]],current_flag=True,ss=ss)
            # Committor
            fig,ax = self.plot_field_2d(data,self.comm_fwd[ss,0],self.chom[ss],magu_x,fieldname=r"$q^+$",fun0name=fun0name,fun1name=fun1name,std_flag=True,current_flag=False,thetaj=(magu_yj,magu_xpj,magu_ypj),theta_ab=magu_ab,ss=ss,units=units,unit_symbols=unit_symbols,cbar_orientation='horizontal')
            fig.savefig(join(self.savefolder,"comm_fwd_{}".format(fname_suffix_list[i])))
            plt.close(fig)
            fig,ax = self.plot_field_2d(data,1-self.comm_bwd[ss,0],self.chom[ss],magu_x,dirn=-1,fieldname=r"$1-q^-$",fun0name=fun0name,fun1name=fun1name,std_flag=True,current_flag=False,thetaj=(magu_yj,magu_xpj,magu_ypj),theta_ab=magu_ab,ss=ss,units=units,unit_symbols=unit_symbols,cbar_orientation='horizontal')
            fig.savefig(join(self.savefolder,"comm_bwd_{}".format(fname_suffix_list[i])))
            plt.close(fig)
            # Conditional MFPT
            # Forward
            fig,ax = self.plot_field_2d(data,self.mfpt_a_avb_fwd[ss,0],self.chom[ss],magu_x,fieldname=r"$E[\tau_A^+|\tau_A^+<\tau_B^+]$",fun0name=fun0name,fun1name=fun1name,std_flag=True,current_flag=False,theta_ab=magu_ab,ss=ss,units=units,unit_symbols=unit_symbols,cbar_orientation='horizontal')
            fig.savefig((join(self.savefolder,"mfpt_a_avb_fwd_{}".format(fname_suffix_list[i]))).replace('.','p'))
            plt.close(fig)
            fig,ax = self.plot_field_2d(data,self.mfpt_b_ava_fwd[ss,0],self.chom[ss],magu_x,fieldname=r"$E[\tau_B^+|\tau_B^+<\tau_A^+]$",fun0name=fun0name,fun1name=fun1name,std_flag=True,current_flag=False,theta_ab=magu_ab,ss=ss,units=units,unit_symbols=unit_symbols,cbar_orientation='horizontal')
            fig.savefig((join(self.savefolder,"mfpt_b_ava_fwd_{}".format(fname_suffix_list[i]))).replace('.','p'))
            plt.close(fig)
            # Backward
            fig,ax = self.plot_field_2d(data,self.mfpt_a_avb_bwd[ss,0],self.chom[ss],magu_x,fieldname=r"$E[\tau_A^-|\tau_A^->\tau_B^-]$",fun0name=fun0name,fun1name=fun1name,std_flag=True,current_flag=False,theta_ab=magu_ab,ss=ss,units=units,unit_symbols=unit_symbols)
            fig.savefig((join(self.savefolder,"mfpt_a_avb_bwd_{}".format(fname_suffix_list[i]))).replace('.','p'))
            plt.close(fig)
            fig,ax = self.plot_field_2d(data,self.mfpt_b_ava_bwd[ss,0],self.chom[ss],magu_x,fieldname=r"$E[\tau_B^-|\tau_B^->\tau_A^-]$",fun0name=fun0name,fun1name=fun1name,std_flag=True,current_flag=False,theta_ab=magu_ab,ss=ss,units=units,unit_symbols=unit_symbols)
            fig.savefig((join(self.savefolder,"mfpt_b_ava_bwd_{}".format(fname_suffix_list[i]))).replace('.','p'))
            plt.close(fig)
            # Combined
            fig,ax = self.plot_field_2d(data,self.mpt_ab[ss,0],self.chom[ss],magu_x,fieldname=r"$E[\tau_B^+-\tau_A^-|A\to B]$",fun0name=fun0name,fun1name=fun1name,std_flag=True,current_flag=False,theta_ab=magu_ab,ss=ss,units=units,unit_symbols=unit_symbols)
            fig.savefig((join(self.savefolder,"mpt_ab_{}".format(fname_suffix_list[i]))).replace('.','p'))
            plt.close(fig)
            fig,ax = self.plot_field_2d(data,self.mpt_ba[ss,0],self.chom[ss],magu_x,fieldname=r"$E[\tau_A^+-\tau_B^-|B\to A]$",fun0name=fun0name,fun1name=fun1name,std_flag=True,current_flag=False,theta_ab=magu_ab,ss=ss,units=units,unit_symbols=unit_symbols)
            fig.savefig((join(self.savefolder,"mpt_ba_{}".format(fname_suffix_list[i]))).replace('.','p'))
            plt.close(fig)
            # Mean first passage time
            # tau_B^+
            fig,ax = self.plot_field_2d(data,self.mfpt_b_x[ss,0],self.chom[ss],magu_x,fieldname=r"$E[\tau_B^+]$ (days)",fun0name=fun0name,fun1name=fun1name,std_flag=True,current_flag=False,thetaj=(magu_yj,magu_xpj,magu_ypj),theta_ab=magu_ab,ss=ss,units=units,unit_symbols=unit_symbols,cbar_orientation='horizontal')
            fig.savefig(join(self.savefolder,"mfpt_b_fwd_{}".format(fname_suffix_list[i])))
            plt.close(fig)
            # tau_B^-
            fig,ax = self.plot_field_2d(data,self.mfpt_b_bwd[ss,0],self.chom[ss],magu_x,fieldname=r"$E[\tau_B^-]$ (days)",fun0name=fun0name,fun1name=fun1name,std_flag=True,current_flag=False,thetaj=(magu_yj,magu_xpj,magu_ypj),theta_ab=magu_ab,ss=ss,units=units,unit_symbols=unit_symbols,cbar_orientation='horizontal')
            fig.savefig(join(self.savefolder,"mfpt_b_bwd_{}".format(fname_suffix_list[i])))
            plt.close(fig)
            # tau_A^+
            fig,ax = self.plot_field_2d(data,self.mfpt_a_x[ss,0],self.chom[ss],magu_x,fieldname=r"$E[\tau_A^+]$ (days)",fun0name=fun0name,fun1name=fun1name,std_flag=True,current_flag=False,thetaj=(magu_yj,magu_xpj,magu_ypj),theta_ab=magu_ab,ss=ss,units=units,unit_symbols=unit_symbols,cbar_orientation='horizontal')
            fig.savefig(join(self.savefolder,"mfpt_a_fwd_{}".format(fname_suffix_list[i])))
            plt.close(fig)
            # tau_A^-
            fig,ax = self.plot_field_2d(data,self.mfpt_a_bwd[ss,0],self.chom[ss],magu_x,fieldname=r"$E[\tau_A^-]$ (days)",fun0name=fun0name,fun1name=fun1name,std_flag=True,current_flag=False,thetaj=(magu_yj,magu_xpj,magu_ypj),theta_ab=magu_ab,ss=ss,units=units,unit_symbols=unit_symbols,cbar_orientation='horizontal')
            fig.savefig(join(self.savefolder,"mfpt_a_bwd_{}".format(fname_suffix_list[i])))
            plt.close(fig)
            # Densities
            fig,ax = self.plot_field_2d(data,self.chom[ss],np.ones(self.nshort)[ss],magu_x,avg_flag=False,cmap=plt.cm.summer,fieldname=r"$\pi$",fun0name=fun0name,fun1name=fun1name,std_flag=False,current_flag=True,dirn=0,thetaj=(magu_yj,magu_xpj,magu_ypj),theta_ab=magu_ab,logscale=True,abpoints_flag=False,ss=ss,units=units,unit_symbols=unit_symbols,cbar_orientation='horizontal')
            fig.savefig(join(self.savefolder,"pi_{}".format(fname_suffix_list[i])))
            plt.close(fig)
            fig,ax = self.plot_field_2d(data,piab,np.ones(self.nshort)[ss],magu_x,avg_flag=False,cmap=plt.cm.summer,fieldname=r"$\pi_{AB}$",fun0name=fun0name,fun1name=fun1name,std_flag=False,current_flag=True,dirn=1,thetaj=(magu_yj,magu_xpj,magu_ypj),theta_ab=magu_ab,logscale=True,abpoints_flag=False,ss=ss,magu_fw=magu_fw_ab,magu_obs=magu_obs_ab,units=units,unit_symbols=unit_symbols,cbar_orientation='horizontal')
            fig.savefig(join(self.savefolder,"piab_{}".format(fname_suffix_list[i])))
            plt.close(fig)
            fig,ax = self.plot_field_2d(data,piba,np.ones(self.nshort)[ss],magu_x,avg_flag=False,cmap=plt.cm.summer,fieldname=r"$\pi_{BA}$",fun0name=fun0name,fun1name=fun1name,std_flag=False,current_flag=True,dirn=-1,thetaj=(magu_yj,magu_xpj,magu_ypj),theta_ab=magu_ab,logscale=True,abpoints_flag=False,ss=ss,magu_fw=magu_fw_ba,magu_obs=magu_obs_ba,units=units,unit_symbols=unit_symbols,cbar_orientation='horizontal')
            fig.savefig(join(self.savefolder,"piba_{}".format(fname_suffix_list[i])))
            plt.close(fig)
        return
    def compute_comm_ab(self,model,data):
        n = q['Nz']-1
        print("Starting committor calculation")
        self.comm_fwd,self.comm_bwd = self.compute_comm_general(q,data,model.adist,model.bdist)
        print("Forward committor:\n range=({},{})".format(np.min(self.comm_fwd),np.max(self.comm_fwd)))
        print("Backward committor:\n range=({},{})".format(np.min(self.comm_bwd),np.max(self.comm_bwd)))
        self.comm_fwd = np.maximum(0.0,self.comm_fwd)
        self.comm_fwd = np.minimum(1.0,self.comm_fwd)
        self.comm_bwd = np.maximum(0.0,self.comm_bwd)
        self.comm_bwd = np.minimum(1.0,self.comm_bwd)
        return
    def display_comm_ab(self,theta_1d_short,theta_1d_long,theta_2d_short,theta_2d_long,theta_1d_name,theta_2d_names,theta_1d_units,theta_2d_units):
        # Display the change of measure from both DGA and simulation
        print("Displaying committor")
        nlong = theta_1d_long.shape[0]
        shp_2d = np.array([10,10])
        fig,ax = helper.compare_plot_fields_2d(theta_2d_short,theta_2d_long,self.comm_fwd[:,0],1*(self.long_to_label==1),self.chom,np.ones(nlong)/nlong,theta_names=theta_2d_names,u_names=[r"$q^+$ DGA",r"$q^+$ Empirical"],avg_flag=True,logscale=False,shp=shp_2d)
        fig.savefig(join(self.savefolder,"fidelity_qp_2D"))
        plt.close(fig)
        fig,ax = helper.compare_plot_fields_2d(theta_2d_short,theta_2d_long,1-self.comm_bwd[:,0],1-1*(self.long_from_label==-1),self.chom,np.ones(nlong)/nlong,theta_names=theta_2d_names,u_names=[r"$1-q^-$ DGA",r"$1-q^-$ Empirical"],avg_flag=True,logscale=False,shp=shp_2d)
        fig.savefig(join(self.savefolder,"fidelity_qm_2D"))
        plt.close(fig)
        shp_1d = np.array([10])
        fig,ax = helper.compare_plot_fields_1d(theta_1d_short,theta_1d_long,self.comm_fwd[:,0],1*(self.long_to_label==1),self.chom,np.ones(nlong)/nlong,theta_name=theta_1d_name,u_names=[r"$q^+$ DGA",r"$q^+$ Empirical"],avg_flag=True,logscale=False,shp=shp_1d)
        fig.savefig(join(self.savefolder,"fidelity_qp_1D"))
        plt.close(fig)
        fig,ax = helper.compare_plot_fields_1d(theta_1d_short,theta_1d_long,1-self.comm_bwd[:,0],1-1*(self.long_from_label==-1),self.chom,np.ones(nlong)/nlong,theta_name=theta_1d_name,u_names=[r"$1-q^-$ DGA",r"$1-q^-$ Empirical"],avg_flag=True,logscale=False,shp=shp_1d)
        fig.savefig(join(self.savefolder,"fidelity_qm_1D"))
        plt.close(fig)
        return
    def compute_comm_general(self,model,data,function):
        # Compute both forward and backward committors in one direction
        # Be agnostic to the subclass of Function object
        def bdy_dist(x):
            fdx = model.adist(x)
            tdx = model.bdist(x)
            bdx = fdx*tdx/(fdx+tdx)**2
            return bdx
        src_fun = lambda x: np.zeros(len(x))#np.zeros((data.nshort,data.traj_length))
        pot_fun = lambda x: np.zeros(len(x))
        # the boundary function depends on the basis type
        if function.function_type == 'LinearBasis':
            if function.basis_type == 'MSM':
                bdy_fun_fwd = lambda x: 1.0*(model.bdist(x) == 0)
                bdy_fun_bwd = lambda x: 1.0*(model.adist(x) == 0)
            elif function.basis_type == 'PCA':
                bdy_fun_fwd = lambda x: model.adist(x)/(model.adist(x) + model.bdist(x))**2
                bdy_fun_bwd = lambda x: model.bdist(x)/(model.adist(x) + model.bdist(x))**2
            else:
                sys.exit('Choose a linear basis type')
        else:
            sys.exit('Choose a function type')
        function.fit_data(data.X[:,0],bdy_dist)
        self.comm_fwd = function.solve_boundary_value_problem(data,bdy_dist,bdy_fun_fwd,src_fun,pot_fun,dirn=1,weights=np.ones(data.nshort)/data.nshort)
        function.fit_data(data.X[:,-1],bdy_dist)
        self.comm_bwd = function.solve_boundary_value_problem(data,bdy_dist,bdy_fun_bwd,src_fun,pot_fun,dirn=-1,weights=self.chom)
        print("comm_fwd in ({},{})".format(np.min(self.comm_fwd),np.max(self.comm_fwd)))
        print("comm_bwd in ({},{})".format(np.min(self.comm_fwd),np.max(self.comm_bwd)))
        return
    def compute_path_integral_average(self,t_long,x_long,starts,ends,damage):
        # Compute the average over paths of a damage (-V in the Feynman-Kac formula)
        # Both time weighted and not
        M = len(starts) # Number of transition paths
        transit_times = np.zeros(M)
        integrals = np.zeros(M)
        dt = t_long[1]-t_long[0]
        for m in range(M):
            transit_times[m] = t_long[ends[m]] - t_long[starts[m]]
            Vm = damage(x_long[starts[m]:ends[m]])
            #print("Vm.shape = {}".format(Vm.shape))
            integrals[m] = 0.5*dt*(Vm[0] + 2*np.sum(Vm[1:-1]) + Vm[-1])
        avg = np.mean(integrals)
        avg_tweighted = np.sum(integrals*transit_times)/np.sum(transit_times)
        return avg,avg_tweighted
    ...
    def display_dam_moments_abba_validation(self,model,data,theta_1d_fun,theta_2d_fun,theta_1d_name,theta_2d_names,theta_1d_units,theta_2d_units):
        ...
        for i in range(num_moments+1):
            ...
            # Forward-in-time
        return
    def display_dam_moments_ab(self,theta_1d_short,theta_1d_long,theta_2d_short,theta_2d_long,theta_1d_name,theta_2d_names,theta_1d_units,theta_2d_unit):
        print("Displaying MFPT as damage function")
        shp_2d = np.array([10,10])
        # Start with MFPT...but think about ability to do other functions
        nlong = theta_1d_long.shape[0]
        num_moments = self.dam_moments_ax.shape[0]-1
        long_tau_a_bwd = load(join(self.savefolder,"long_tau_a_bwd.npy"))
        long_tau_b_fwd = load(join(self.savefolder,"long_tau_b_fwd.npy"))
        for i in range(num_moments+1):
            long_tau_ax_moment = long_tau_a_bwd**i*(self.long_from_label==-1)
            long_tau_xb_moment = long_tau_b_fwd**i*(self.long_to_label==1)
            print("self.dam_moments_xb[i,:,0] in ({},{}). Num > 0 = {}, mean={}, std={}".format(np.min(self.dam_moments_xb[i,:,0]),np.max(self.dam_moments_xb[i,:,0]),np.sum(self.dam_moments_xb[i,:,0]>0),np.mean(self.dam_moments_xb[i,:,0]),np.std(self.dam_moments_xb[i,:,0])))
            fig,ax = helper.compare_plot_fields_2d(theta_2d_short,theta_2d_long,self.dam_moments_xb[i,:,0],long_tau_xb_moment,self.chom,np.ones(nlong)/nlong,theta_names=theta_2d_names,u_names=[r"$E[(\tau_B^+)^{%d}1_B(X(\tau^+))]$ DGA"%i,r"$E[(\tau_B^+)^{%d}1_B(X(\tau^+))]$ Empirical"%i],avg_flag=True,logscale=(i>0),shp=shp_2d)
            fig.savefig(join(self.savefolder,"fidelity_dam_b%d+_2D"%i))
            plt.close(fig)
            fig,ax = helper.compare_plot_fields_2d(theta_2d_short,theta_2d_long,self.dam_moments_ax[i,:,0],long_tau_ax_moment,self.chom,np.ones(nlong)/nlong,theta_names=theta_2d_names,u_names=[r"$E[(\tau_A^-)^{%d}1_A(X(\tau^-))]$ DGA"%i,r"$E[(\tau_A^-)^{%d}1_A(X(\tau^-))]$ Empirical"%i],avg_flag=True,logscale=(i>0),shp=shp_2d)
            fig.savefig(join(self.savefolder,"fidelity_dam_a%d-_2D"%i))
            plt.close(fig)
        return
    def compute_conditional_mfpt_ab_moments(self,model,data,function):
        num_moments = 1
        # Compute conditional first passage times A->x->B and B->x->A
        bdy_dist = lambda x: np.minimum(model.adist(x),model.bdist(x))
        src_fun = lambda x: np.zeros(len(x))
        bdy_fun_b = lambda x: 1.0*(model.bdist(x) == 0) #TODO: adjust by basis
        bdy_fun_a = lambda x: 1.0*(model.adist(x) == 0)
        pot_fun = lambda x: -np.ones(len(x))
        # Forward times: x->A, x->B
        function.fit_data(data.X[:,0],bdy_dist)
        mfpt_moments_xa = function.solve_damage_function_moments(data,bdy_dist,bdy_fun_a,src_fun,pot_fun,dirn=1,weights=np.ones(data.nshort)/data.nshort,num_moments=num_moments)
        print("mfpt_moments_xa in ({},{})".format(np.min(mfpt_moments_xa),np.max(mfpt_moments_xa)))
        mfpt_moments_xb = function.solve_damage_function_moments(data,bdy_dist,bdy_fun_b,src_fun,pot_fun,dirn=1,weights=np.ones(data.nshort)/data.nshort,num_moments=num_moments)
        print("mfpt_moments_xb in ({},{})".format(np.min(mfpt_moments_xb),np.max(mfpt_moments_xb)))
        # Backward times: A->x, B->x
        function.fit_data(data.X[:,-1],bdy_dist)
        mfpt_moments_ax = function.solve_damage_function_moments(data,bdy_dist,bdy_fun_a,src_fun,pot_fun,dirn=-1,weights=self.chom,num_moments=num_moments)
        print("mfpt_moments_ax in ({},{})".format(np.min(mfpt_moments_ax),np.max(mfpt_moments_ax)))
        mfpt_moments_bx = function.solve_damage_function_moments(data,bdy_dist,bdy_fun_b,src_fun,pot_fun,dirn=-1,weights=self.chom,num_moments=num_moments)
        print("mfpt_moments_bx in ({},{})".format(np.min(mfpt_moments_bx),np.max(mfpt_moments_bx)))
        # Put them together using the product rule (this may only work with boundary functions that are unity)
        mpt_ab = np.zeros((num_moments,self.nshort,data.traj_length))
        mpt_ba = np.zeros((num_moments,self.nshort,data.traj_length))
        # First moment
        mpt_ab[0] = mfpt_moments_ax[1]*mfpt_moments_xb[0] + mfpt_moments_ax[0]*mfpt_moments_xb[1]  # Because both zeroth moments are 1
        print("sum(mpt_ab[0]) = {}".format(np.sum(mpt_ab[0])))
        mpt_ba[0] = mfpt_moments_bx[1]*mfpt_moments_xa[0] + mfpt_moments_bx[0]*mfpt_moments_xa[1]  # Because both zeroth moments are 1
        print("sum(mpt_ba[0]) = {}".format(np.sum(mpt_ba[0])))
        #if num_moments > 1:
        #    mpt_ab[1] = mfpt_moments_ax[1] + 2*mfpt_moments_ax[0]*mfpt_moments_xb[0] + mfpt_moments_xb[1]
        #    mpt_ba[1] = mfpt_moments_bx[1] + 2*mfpt_moments_bx[0]*mfpt_moments_xa[0] + mfpt_moments_xa[1]
        # Take time-weighted averages
        self.mpt_ab_tweighted = np.sum(mpt_ab[0,:,0]*self.chom)/np.sum(self.comm_fwd[:,0]*self.comm_bwd[:,0]*self.chom)
        self.mpt_ba_tweighted = np.sum(mpt_ba[0,:,0]*self.chom)/np.sum((1-self.comm_fwd[:,0])*(1-self.comm_bwd[:,0])*self.chom)
        self.mfpt_xa = mfpt_moments_xa[1]
        self.mfpt_xb = mfpt_moments_xb[1]
        self.mfpt_ax = mfpt_moments_ax[1]
        self.mfpt_bx = mfpt_moments_bx[1]
        self.mpt_ab = mpt_ab[0]
        self.mpt_ba = mpt_ba[0]
        return 
    ...
    def plot_transition_states(self,model,data):
        ...
        plt.close(fig)
        ## B -> A
        #reac_dens_idx = np.zeros(len(qlevels),dtype=int)
        #real_qlevels = np.zeros(len(qlevels))
        #for i in range(len(qlevels)):
        #    reac_dens_idx[i:i+1],reac_dens_weights,ans2 = self.maximize_rflux_on_surface(model,data,comm_fwd.reshape((Nx,Nt,1)),1-comm_bwd,1-comm_fwd,weight,qlevels[i],0.02,1)
        #    print("ans2.shape = {}".format(ans2.shape))
        #    real_qlevels[i:i+1] = ans2.flatten()
        #    #reac_dens_idx[i:i+1],reac_dens_weights,real_qlevels[i:i+1] = self.maximize_rdens_on_surface(1-comm_bwd,1-comm_fwd,weight,comm_fwd,qlevels[i],0.05,1)
        #fig,ax = model.plot_multiple_states(data.X[reac_dens_idx,0],real_qlevels,r"q^+")
        #ax.set_title(r"$B\to A$ transition states")
        #fig.savefig(join(self.savefolder,"trans_states_ba"))
        #plt.close(fig)
        return
