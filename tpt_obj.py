...
class TPT:
    def __init__(self,algo_params,physical_param_folder,long_simfolder,short_simfolder,savefolder):
        ...
        self.lag_halftime_current_display = algo_params['lag_halftime_current_display']
    ...
    def plot_field_long(self,model,data,key_combo,color_combo,zi_combo,suffix_combo,tmax=70,time_unit_symbol=None,phases=['aa','ab','ba','bb'],density_1d_flag=False):
        ...
        for i_z in range(len(zi_combo)):
            ...
            for i_key in range(len(key_combo)):
                ...
                max_duration = int(80/(t_long[1]-t_long[0]))
    ...
    def display_dam_moments_abba_current(self,model,data,profkey0,profkey1,idx_combo_list,suffix_combo_list,same_bounds_flag=False,square_bounds_flag=False):
        ...
        prof0_x = funlib[profkey0]["fun"](data.X[:,:Nt_max].reshape((Nx*Nt_max,xdim)))
        prof0dim = int(prof0_x.size / (Nx*Nt_max))
        prof0_a,prof0_b = funlib[profkey0]["fun"](model.tpt_obs_xst).reshape((2,prof0dim))
        prof1_x = funlib[profkey1]["fun"](data.X[:,:Nt_max].reshape((Nx*Nt_max,xdim)))
        prof1dim = int(prof1_x.size / (Nx*Nt_max))
    ...
    def display_casts_abba(self,model,data,profkey0,profkey1,idx_combo_list,suffix_combo_list,same_bounds_flag=False,square_bounds_flag=False): #theta_2d_abbs):
        ...
        Nt_max = int(np.ceil(np.argmin(np.abs(data.t_x - self.lag_time_current_display)))) + 10
        ...
        print(f"About to compute {profkey0} and {profkey1} functions")
        prof0_x = funlib[profkey0]["fun"](data.X[:,:Nt_max].reshape((Nx*Nt_max,xdim)))
        prof0dim = int(prof0_x.size / (Nx*Nt_max))
        prof0_a,prof0_b = funlib[profkey0]["fun"](model.tpt_obs_xst).reshape((2,prof0dim))
        prof1_x = funlib[profkey1]["fun"](data.X[:,:Nt_max].reshape((Nx*Nt_max,xdim)))
        ...
        for i_com in range(len(idx_combo_list)):
            ...
            theta_x = np.array([prof0_x[:,zi0], prof1_x[:,zi1]]).T.reshape((Nx,Nt_max,2))
            theta_xst = np.array([[prof0_a[zi0],prof1_a[zi1]],[prof0_b[zi0],prof1_b[zi1]]])
            theta_2d_names = [r"%s %s"%(funlib[profkey0]["name"],suffix_combo_list[i_com][0]),r"%s %s"%(funlib[profkey1]["name"],suffix_combo_list[i_com][1])]
            theta_2d_units = [funlib[profkey0]["units"],funlib[profkey1]["units"]]
            theta_2d_unit_symbols = [funlib[profkey0]["unit_symbol"],funlib[profkey1]["unit_symbol"]]
            weight = self.chom
            print(f"keys = {keys}")
            for k in range(len(keys)):
                ...
                for j in range(min(2,num_moments)):
                    ...
                    if phase_flags['ab']:
                        ...
                        plt.close(fig)
                        print(f"Done saving fig with suffix {fsuff}")
                        # Plot the current as well
                    # ---------------------------------
                    # B->A
                    ...
                    if phase_flags['bx']:
                        ...
                        ax.set_ylim(prof1_bounds*funlib[profkey1]["units"])
                        print(f"Done plotting field 2d")
                        fsuff = 'cast_%s%d_bx_th0%szi%i_th1%szi%i'%(model.dam_dict[keys[k]]['abb_full'],j,funlib[profkey0]["abbrv"],zi0,funlib[profkey1]["abbrv"],zi1)
                        fig.savefig(join(self.savefolder,fsuff),bbox_inches="tight",pad_inches=0.2)
                        plt.close(fig)
                        print(f"Done saving fig")
                    # ---------------------------------
                    ...
                    if phase_flags['ax']:
                        ...
                        ax.set_ylim(prof1_bounds*funlib[profkey1]["units"])
                        print(f"Done plotting field 2d")
                        fsuff = 'cast_%s%d_ax_th0%szi%i_th1%szi%i'%(model.dam_dict[keys[k]]['abb_full'],j,funlib[profkey0]["abbrv"],zi0,funlib[profkey1]["abbrv"],zi1)
                        fig.savefig(join(self.savefolder,fsuff),bbox_inches="tight",pad_inches=0.2)
                        plt.close(fig)
                        print(f"Done saving fig")
        return
    ...
    def display_1d_densities(self,model,data,theta_1d_key,theta_1d_orientations,fig=None,ax=None,include_reactive=True,phases=['aa','ab','bb','ba'],save_flag=False):
        ...
        for k in range(len(theta_1d_key)):
            ...
            if save_flag:
                ...
                plt.close(fig)
        return
    def display_2d_currents(self,model,data,profkey0,profkey1,idx_combo_list,same_bounds_flag=True): #theta_2d_abbs):
        funlib = model.observable_function_library()
        ...
        return
    def display_change_of_measure_current(self,model,data,theta_x,theta_2d_names,theta_2d_units,theta_2d_unit_symbols,theta_2d_abbs):
    ...
    def tendency_during_transition(self,model,data,X,theta_fun,comm_bwd,comm_fwd,dirns):
        ...
        lag_halftime = self.lag_halftime_current_display
        ti0 = np.argmin(np.abs(lag_halftime - data.t_x))
        ...
        print(f"ti0 = {ti0}. tidn range = ({tidn.min()},{tidn.max()}). tiup range = ({tiup.min(),tiup.max()})")
    ...
    def project_current_new(self,model,data,theta_x,comm_bwd,comm_fwd):
        ...
        data.insert_boundaries_fwd(bdy_dist_x,self.lag_halftime_current_display,self.lag_time_current_display)
        data.insert_boundaries_bwd(bdy_dist_x,self.lag_halftime_current_display,0)
        ti0 = np.argmin(np.abs(data.t_x - self.lag_halftime_current_display))
        Jtheta_up = (
            (
                self.chom*
                comm_bwd[:,ti0]*
                comm_fwd[np.arange(Nx),data.first_exit_idx_fwd]/
                (data.t_x[data.first_exit_idx_fwd] - data.t_x[ti0])
            ) * (
                theta_x[np.arange(Nx),data.first_exit_idx_fwd] - theta_x[:,ti0]
                ).T
            ).T
        Jtheta_dn = (
            (
                self.chom*
                comm_fwd[np.arange(Nx),ti0]*
                comm_bwd[np.arange(Nx),data.first_exit_idx_bwd]/
                (data.t_x[ti0] - data.t_x[data.first_exit_idx_bwd])
            ) * (
                theta_x[np.arange(Nx),ti0] - theta_x[np.arange(Nx),data.first_exit_idx_bwd]
                ).T
            ).T
    ...
    def maximize_rflux_on_surface(self,model,data,theta_x,comm_bwd,comm_fwd,weight,theta_level,theta_tol,max_num_states,frac_of_max):
        print("theta_x.shape = {}".format(theta_x.shape))
        Jup,Jdn,ti0 = self.project_current_new(model,data,theta_x,comm_bwd,comm_fwd)
        print("Jup.shape = {}, Jdn.shape = {}".format(Jup.shape,Jdn.shape))
        print("theta_level = {}".format(theta_level))
        Jup = Jup.flatten()
        Jdn = Jdn.flatten()
        nnidx = np.where(np.isnan(theta_x[:,ti0,0]) + np.isnan(Jup) + np.isnan(Jdn) == 0)[0]
        theta_discrepancy = np.sqrt(np.sum((theta_x[nnidx,ti0] - theta_level)**2, 1))
        ...
        return list(idx[rflux_max_idx]),list(rflux[rflux_max_idx]),theta_x[idx[rflux_max_idx],ti0]
    ...
    def plot_transition_states_ensttend(self,model,data,xlim_flag=True,lap_flag=True):
        ...
        #ss = np.arange(data.X.shape[0])
        X = data.X[ss]
        Nx,Nt,xdim = X.shape
        n = model.q['Nz']-1
        comm_fwd = self.dam_moments['one']['xb'][0,ss,:]
        comm_bwd = self.dam_moments['one']['ax'][0,ss,:]
        eta_B = -self.dam_moments['one']['xb'][1,ss,:]*(comm_fwd > 1e-1)/(comm_fwd + 1.0*(comm_fwd <= 1e-1))
        eta_B[comm_fwd <= 1e-1] = np.nan
        chom = self.chom[ss]
        ...
        eta_labels = [r"$-\eta_B^+=%.2f$"%(0.5*(
            eta_levels[i]-eta_tol_list[i] + 
            eta_levels[i]+eta_tol_list[i])) 
            for i in range(len(eta_levels))]
        qlevel_idx = []
        lag_time_max = self.lag_time_current_display
        lag_halftime = self.lag_halftime_current_display
        ti0 = np.argmin(np.abs(lag_halftime - data.t_x))
        for qi in range(len(qp_levels)):
            ...
            idx_qi = np.where(np.abs(comm_fwd[:,ti0] - qp_levels[qi]) < qp_tol)[0]
            qlevel_idx += [idx_qi]
        etalevel_idx = []
        for ti in range(len(eta_levels)):
            eta_tol = eta_tol_list[ti]
        ...
        if (not compute_transdict_flag):
            transdict = pickle.load(open(join(self.savefolder,"transdict"),"rb"))
            quantile_midranges = transdict["quantile_midranges"]
        else:
            quantile_midranges = np.array([0.25,0.5,0.9])
            transdict = dict({"quantile_midranges": quantile_midranges})
            transdict["progress"] = dict({
                "committor": dict({
                    "levels": qp_levels,
                    "tol_list": qp_tol_list,
            ...
            for key in list(keys_prof):
                ...
                print(f"theta_x.shape = {theta_x.shape}")
                thdim = theta_x.shape[-1]
                transdict[key] = dict({"leadtime": dict(), "committor": dict()})
                for prog in ["leadtime","committor"]:
                    print(f"Starting progress coords {prog}")
                    transdict[key][prog]["snapshot"] = dict({"stochastic": dict(), "xlim": np.zeros(2)})
                    transdict[key][prog]["tendency"] = dict({"stochastic": dict(), "deterministic": dict(), "xlim": np.zeros(2)})
                    for dirn in dirns:
                        transdict[key][prog]["snapshot"]["stochastic"][dirn] = np.zeros((len(transdict["progress"][prog]["levels"]),1+2*len(quantile_midranges),thdim))
                        transdict[key][prog]["tendency"]["stochastic"][dirn] = np.zeros((len(transdict["progress"][prog]["levels"]),thdim))
                        transdict[key][prog]["tendency"]["deterministic"][dirn] = np.zeros((len(transdict["progress"][prog]["levels"]),thdim))
                    for qi in range(len(transdict["progress"][prog]["levels"])):
                        idx = np.array(transdict["progress"][prog]["level_idx"][qi]) 
                        print(f"At progress level {qi} = {transdict['progress'][prog]['levels'][qi]}, len(idx) = {len(idx)}")
                        if len(idx) > 0:
                            for dirn in dirns:
                                ...
                                # --------------- Diagnosis: plot the points contributing to these idx -------------------------
                                fig,ax = plt.subplots()
                                idx_c0 = funlib["vTint"]["fun"](X[idx,ti0])[:,model.q['zi']]*funlib["vTint"]["units"]
                                idx_c1 = funlib["U"]["fun"](X[idx,ti0])[:,model.q['zi']]*funlib["U"]["units"]
                                ax.set_xlabel(funlib["vTint"]["abbrv"])
                                ax.set_ylabel(funlib["U"]["abbrv"])
                                idx_ss = np.random.choice(np.arange(len(idx)),size=min(len(idx),10000),replace=False)
                                ax.scatter(idx_c0[idx_ss],idx_c1[idx_ss],color='black',alpha=weights[idx_ss]/np.max(weights[idx_ss]),marker='.')
                                ax.set_xlim([-10e3,50e3])
                                ax.set_ylim([-50,100])
                                fig.savefig(join(self.savefolder,f"trans_state_diagnosis_prog{prog}_qi{qi}"))
                                plt.close(fig)
                                # -------------------------------------------------------
                                # Snapshot, stochastic
                                for d in range(thdim):
                                    order = np.argsort(theta_x[idx,ti0,d])
                                    cdf = np.cumsum(weights[order])
                                    for k in range(len(quantile_midranges)):
                                        alpha = (1 - quantile_midranges[k])/2
                                        transdict[key][prog]["snapshot"]["stochastic"][dirn][qi,2*k,d] = theta_x[idx[order[np.where(cdf >= alpha)[0][0]]],ti0,d]
                                        transdict[key][prog]["snapshot"]["stochastic"][dirn][qi,2*k+1,d] = theta_x[idx[order[np.where(cdf >= 1-alpha)[0][0]]],ti0,d]
                                transdict[key][prog]["snapshot"]["stochastic"][dirn][qi,-1] = np.nansum((theta_x[idx,ti0].T * weights).T, axis=0) 
                                # Tendency, stochastic
                                weights = chom[idx]/np.sum(chom[idx]*qm*qp) 
                                transdict[key][prog]["tendency"]["stochastic"][dirn][qi] = np.nansum((L[dirn][idx].T * weights).T, axis=0)
                                # Tendency, deterministic  
                                weights = chom[idx]*qm*qp/np.sum(chom[idx]*qm*qp)
                                transdict[key][prog]["tendency"]["deterministic"][dirn][qi] = np.nansum((L["deterministic"][idx].T * weights).T, axis=0)
                                # Axis limits
                                transdict[key][prog]["snapshot"]["xlim"][0] = min(
                                        transdict[key][prog]["snapshot"]["xlim"][0],
                                        np.min(transdict[key][prog]["snapshot"]["stochastic"][dirn][qi,:-3,:]), # Exclude the largest quantile envelope
                                        )
                                transdict[key][prog]["snapshot"]["xlim"][1] = max(
                                        transdict[key][prog]["snapshot"]["xlim"][1],
                                        np.max(transdict[key][prog]["snapshot"]["stochastic"][dirn][qi,:-3,:]),
                                        )
                                transdict[key][prog]["tendency"]["xlim"][0] = min(
                                        transdict[key][prog]["tendency"]["xlim"][0],
                                        np.min(transdict[key][prog]["tendency"]["stochastic"][dirn][qi]),
                                        np.min(transdict[key][prog]["tendency"]["deterministic"][dirn][qi])
                                        )
                                transdict[key][prog]["tendency"]["xlim"][1] = max(
                                        transdict[key][prog]["tendency"]["xlim"][1],
                                        np.max(transdict[key][prog]["tendency"]["stochastic"][dirn][qi]),
                                        np.max(transdict[key][prog]["tendency"]["deterministic"][dirn][qi])
                                        )
        ...
        if plot_profile_transdict_flag:
            for key in keys_prof:
                abbrv = funlib[key]["abbrv"]
                for prog in ["committor","leadtime"]:
                    ...
                    for qi in range(len(transdict["progress"][prog]["levels"])):
                        ...
                        for dirn in dirns:
                            ...
                            if plot_quantile_flag[dirn]:
                                for k in range(len(quantile_midranges)):
                                    ax[0].fill_betweenx(z, transdict[key][prog]["snapshot"]["stochastic"][dirn][qi,2*k]*funlib[key]["units"], x2=transdict[key][prog]["snapshot"]["stochastic"][dirn][qi,2*k+1]*funlib[key]["units"], color=dirn_cmaps[dirn](0.75*(1-k/len(quantile_midranges))), zorder=-1-k, edgecolor=None)
                            if plot_snapshot_flag[dirn]:
                                h, = ax[0].plot(transdict[key][prog]["snapshot"]["stochastic"][dirn][qi,-1]*funlib[key]["units"], z, color=dirn_colors[dirn], label=dirn_labels[dirn])
                                handles["snapshot"] += [h]
                            # Tendencies 
                            if plot_tendency_flag[dirn]:
                                # Stochastic
                                h, = ax[1].plot(transdict[key][prog]["tendency"]["stochastic"][dirn][qi]*funlib[key]["units"]/model.q["time"], z, color=dirn_colors[dirn], label=dirn_labels[dirn])
                                handles["tendency"] += [h]
                                # Deterministic
                                ax[1].plot(transdict[key][prog]["tendency"]["deterministic"][dirn][qi]*funlib[key]["units"]/model.q["time"], z, color=dirn_colors[dirn], linestyle='--')
                                if lap_flag and (dirn in lap.keys()):
                                    h, = ax[0].plot(lap[dirn][key][prog]["snapshot"][qi]*funlib[key]["units"], z, color="cyan", label=r"Min-action")
                                    handles["snapshot"] += [h]
                                    h, = ax[1].plot(lap[dirn][key][prog]["tendency"][qi]*funlib[key]["units"]/model.q["time"], z, color="cyan", label=r"Min-action")
                                    handles["tendency"] += [h]
                        # Set axis limits 
                        if xlim_flag:
                            ax[0].set_xlim(transdict[key][prog]["snapshot"]["xlim"]*funlib[key]["units"])
                            ax[1].set_xlim(transdict[key][prog]["tendency"]["xlim"]*funlib[key]["units"]/model.q["time"])
                        ...
                        ax[0].set_title(r"%s at %s"%(funlib[key]['name'],transdict["progress"][prog]["labels"][qi]),fontdict=font)
                        ax[1].set_title(r"%s tendencies at %s"%(funlib[key]['name'],transdict["progress"][prog]["labels"][qi]),fontdict=font)
                        ...
                        fig.savefig(join(self.savefolder,f"trans_state_profile_prog{prog}_{dirn_combo_str}_{abbrv}_{qi}_Nx{Nx}_xlim{int(xlim_flag)}").replace(".","p"))
                        plt.close(fig)
                    # ---------------------------------------------------------------------
        ...
        if plot_timeseries_transdict_flag:
            for dirn in ["ab"]:
                for key in ["U","vT"]:
                    ...
                    for prog in ["committor","leadtime"]:
                        ylim_prescribed = ([
                            np.min(transdict[key][prog]["snapshot"]["stochastic"][dirn][:,:,zi_list])*funlib[key]["units"],
                            np.max(transdict[key][prog]["snapshot"]["stochastic"][dirn][:,:,zi_list])*funlib[key]["units"],
                            ])
                        if key in ["U","vT"]:
                            ylim_prescribed[0] = 0.0
                        for i_alt,alt in enumerate(alt_list):
                            # Big question: do we want to plot committor or lead time in the horizontal
                            zi = zi_list[i_alt]
                            fig,ax = plt.subplots(figsize=(6,6)) #, sharex=True)
                            handles = []
                            # Stochastic 
                            h, = ax.plot(transdict["progress"][prog]["levels"],transdict[key][prog]["snapshot"]["stochastic"][dirn][:,-1,zi]*funlib[key]["units"], color=dirn_colors[dirn], linestyle='-', label=r"%s (%s)"%(funlib[key]["name"],dirn_labels[dirn]))
                            handles += [h]
                            for k in range(len(quantile_midranges)):
                                ylohi = transdict[key][prog]["snapshot"]["stochastic"][dirn][:,2*k:2*k+2,zi] * funlib[key]["units"]
                                ax.fill_between(transdict["progress"][prog]["levels"],ylohi[:,0],ylohi[:,1], color=plt.cm.binary(0.75*(1-k/len(quantile_midranges))), zorder=-k)
                            if lap_flag and (dirn in lap.keys()):
        ...
        if plot_analysis_transdict_flag:
            for prog in ["committor","leadtime"]:
                for dirn in ["ab"]:
                    ...
                    GE = transdict["gramps_plus_enstrophy"][prog]["snapshot"]["stochastic"][dirn][:,:,zi_list]*funlib["gramps_plus_enstrophy"]["units"]
                    LGE = transdict["gramps_plus_enstrophy"][prog]["tendency"]["stochastic"][dirn][:,zi_list]*funlib["gramps_plus_enstrophy"]["units"]/q["time"]
                    GEdot = transdict["gramps_plus_enstrophy"][prog]["tendency"]["deterministic"][dirn][:,zi_list]*funlib["gramps_plus_enstrophy"]["units"]/q["time"]
                    G = transdict["gramps"][prog]["snapshot"]["stochastic"][dirn][:,:,zi_list]*funlib["gramps"]["units"]
                    LG = transdict["gramps"][prog]["tendency"]["stochastic"][dirn][:,zi_list]*funlib["gramps"]["units"]/q["time"]
                    Gdot = transdict["gramps"][prog]["tendency"]["deterministic"][dirn][:,zi_list]*funlib["gramps"]["units"]/q["time"]
                    E = transdict["enstrophy"][prog]["snapshot"]["stochastic"][dirn][:,:,zi_list]*funlib["enstrophy"]["units"]
                    LE = transdict["enstrophy"][prog]["tendency"]["stochastic"][dirn][:,zi_list]*funlib["enstrophy"]["units"]/q["time"]
                    Edot = transdict["enstrophy"][prog]["tendency"]["deterministic"][dirn][:,zi_list]*funlib["enstrophy"]["units"]/q["time"]
                    RBe = transdict["gramps_relax"][prog]["snapshot"]["stochastic"][dirn][:,-1,zi_list]*funlib["gramps_relax"]["units"]
                    D = transdict["diss"][prog]["snapshot"]["stochastic"][dirn][:,-1,zi_list]*funlib["diss"]["units"]
                    BeVQ = transdict["dqdy_times_vq"][prog]["snapshot"]["stochastic"][dirn][:,-1,zi_list]*funlib["dqdy_times_vq"]["units"]
                    # Set y axis limits 
                    ylim_prescribed = [
                            [
                    ...
                    for c in range(ax.shape[1]):
                        ax[1,c].set_xlabel(transdict["progress"][prog]["symbol"],fontdict=ffont)
                    ...
                    # Save 
                    fig.savefig(join(self.savefolder,f"trans_state_analysis_prog{prog}_{dirn}"))
                    plt.close(fig)
        return
